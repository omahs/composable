...
fn backing_for_should_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(propose_set_balance_and_note(1, 2, 2, BTC));
		assert_ok!(propose_set_balance_and_note(1, 4, 4, BTC));
		assert_ok!(propose_set_balance_and_note(1, 3, 3, BTC));
...
fn deposit_for_proposals_should_be_taken() {
	new_test_ext().execute_with(|| {
		assert_ok!(propose_set_balance_and_note(1, 2, 5, BTC));
...
fn proposal_with_deposit_below_minimum_should_not_work() {
	new_test_ext().execute_with(|| {
		assert_noop!(propose_set_balance(1, 2, 0, BTC), Error::<Test>::ValueLow);
	});
}
...
fn poor_proposer_should_not_work() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			propose_set_balance(1, 2, 11, BTC),
...
fn poor_seconder_should_not_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(propose_set_balance_and_note(2, 2, 11, BTC));
		assert_noop!(
...
fn blacklisting_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			propose_set_balance_and_note(1, 2, 2, BTC),
