	...
	impl<T: Config> Pallet<T> {
		...
		pub fn deposit_is_within_nonempty_pools_deposit_bounds(
			...
			// depositing_into_a_non_empty_pool_with_duplicate_deposits_correctly_mints_lp_tokens
			{
				...
				// let reserve_increase: u128 = multiply_by_rational_with_rounding(1u128,
				// 	deposit,
				// 	reserve
				...
				// 	None => 0_128,
				// 	Some(percent) => multiply_by_rational_with_rounding(1u128,
		...
		pub fn deposit_matches_underlying_value_distribution(
			...
			let deposit_value_distribution = multiply_by_rational_with_rounding(1, deposit, deposit_total, Rounding::Down)
				.ok_or(ArithmeticError::Overflow)?;

			let reserve: BalanceOf<T> = Self::balance_of(pool_id, &asset)?;
			let reserve: u128 = <T::Convert as Convert<BalanceOf<T>, u128>>::convert(reserve);
			let reserve_total = <T::Convert as Convert<BalanceOf<T>, u128>>::convert(reserve_total);
			let reserve_value_distribution = multiply_by_rational_with_rounding(1, reserve, reserve_total, Rounding::Down)
				.ok_or(ArithmeticError::Overflow)?;
			...
			let margin_of_error: u128 =
				multiply_by_rational_with_rounding(reserve_value_distribution, epsilon, one, Rounding::Down)
					.ok_or(ArithmeticError::Overflow)?;
	...
	impl<T: Config> Pallet<T> {
		...
		fn lps_share_of_asset(
			...
			let lp_share_of_asset =
				multiply_by_rational_with_rounding(pool_balance_of_token, lp_amount, lp_circulating_supply, Rounding::Down)
					.ok_or(ArithmeticError::Overflow)?;
