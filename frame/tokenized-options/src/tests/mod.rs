...
impl VaultInitializer for sp_io::TestExternalities {
	...
	fn initialize_all_vaults(mut self) -> Self {
		self.execute_with(|| {
			ASSETS_WITH_USDC.into_iter().for_each(|asset_id| {
...
pub fn random_option_type() -> impl Strategy<Value = OptionType> {
	prop::sample::select([OptionType::Call, OptionType::Put].as_slice())
}
...
pub fn random_option_config(
	...
	(random_asset(), balance_strategy, random_option_type(), random_epoch(start_rng, duration_rng))
		.prop_map(|(base_asset_id, strike_price, option_type, epoch)| {
