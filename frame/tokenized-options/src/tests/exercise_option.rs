...
pub fn exercise_option_success_checks(option_id: AssetId, option_amount: Balance, who: Public) {
	...
	assert_eq!(updated_total_issuance, initial_total_issuance - option_amount);
}
...
#[test]
fn test_exercise_option_success() {
	ExtBuilder::default()
		.initialize_balances(Vec::from([
			(ALICE, BTC, 10 * UNIT),
			(ALICE, USDC, 500000 * UNIT),
		...
		.execute_with(|| {
			...
			// Settle options
			assert_ok!(TokenizedOptions::settle_options());
			let protocol_amount = Assets::balance(BTC, &protocol_account);
