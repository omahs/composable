...
describe("HAL02 [Oracle] Tests", function () {
    ...
    describe("HAL02: Adding stakes", function () {
        it("HAL02: Adding stake 1", async function () {
            ...
            const { data: [result] } = await (0, addStakeTests_1.txOracleAddStakeSuccessTest)(api, walletHAL02_1, stake);
        ...
        it("HAL02: Adding stake 2", async function () {
            this.timeout(2 * 60 * 1000);
            const stake = api.createType("u128", 2500000000000);
            const { data: [result] } = await (0, addStakeTests_1.txOracleAddStakeSuccessTest)(api, walletHAL02_2, stake);
            (0, chai_1.expect)(result).to.not.be.an("Error");
            (0, chai_1.expect)(result.toString()).to.be.equal(api.createType("AccountId32", walletHAL02_3.publicKey).toString());
        });
        it("HAL02: Adding stake 3", async function () {
            this.timeout(2 * 60 * 1000);
            const stake = api.createType("u128", 2500000000000);
            const { data: [result] } = await (0, addStakeTests_1.txOracleAddStakeSuccessTest)(api, walletHAL02_3, stake);
            (0, chai_1.expect)(result).to.not.be.an("Error");
            (0, chai_1.expect)(result.toString()).to.be.equal(api.createType("AccountId32", walletHAL02_4.publicKey).toString());
        });
        it("HAL02: Adding stake 4", async function () {
            this.timeout(2 * 60 * 1000);
            const stake = api.createType("u128", 2500000000000);
            const { data: [result] } = await (0, addStakeTests_1.txOracleAddStakeSuccessTest)(api, walletHAL02_4, stake);
            (0, chai_1.expect)(result).to.not.be.an("Error");
            (0, chai_1.expect)(result.toString()).to.be.equal(api.createType("AccountId32", controllerWallet.publicKey).toString());
        });
    });
