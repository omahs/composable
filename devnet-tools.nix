{ self, ... }: {
  perSystem = { config, self', inputs', pkgs, system
    , zombienet-rococo-local-composable-config, writeZombienetShellApplication
    , zombienet-to-ops, ... }: {
      _module.args.devnetTools = rec {

        mk-devnet = { lib, chain-spec }:
          let
            config = zombienet-rococo-local-composable-config {
              chain = "${chain-spec}";
            };
            script =
              writeZombienetShellApplication "devnet-${chain-spec}" config;

          in {
            inherit chain-spec script;
            config = zombienet-to-ops config;
          };
